...
const SOWS_BY_PROJECT: Record<string, SowRow[]> = {
  ...
  'p-3': [
    { id: 's-3', sowNumber: 'SOW-1055', title: 'Support retainer renewal', status: 'DRAFT', version: 1, updatedAt: '2026-07-27' },
  ],
};
...
export function ProjectsPage() {
  ...
  return (
    ...
    <div className="min-w-0 flex-1">
      <PageHeader
        title="Projects"
        description="Work tracked against a client, with a single owner in your organization."
        actions={
          <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent title={editing ? 'Edit project' : 'New project'} className="max-w-lg">
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  ...
                  <div>
                    <Label htmlFor="project-end">Expected end</Label>
                    <Input
                      id="project-end"
                      type="date"
                      value={form.endDate}
    ...
    {selectedProject && (
      ...
      >
        ...
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <div className="mt-1 text-sm text-muted-foreground">
              SOWs for {clientName(selectedProject.clientId)}
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="mt-6 border-t border-border pt-4">
          <h3 className="mb-3 text-sm font-semibold text-foreground">Client Requirements</h3>
          {(() => {
            const projectComments = comments[selectedProject.id] ?? [];
            return projectComments.length === 0 ? (
              <p className="mb-3 text-xs text-muted-foreground">No new requirements noted yet.</p>
            ) : (
          ...
          <Textarea
            placeholder="Log a new requirement from the client..."
            rows={3}
            value={newComment}
