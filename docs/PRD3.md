...
- **Diagrammatic workflow preview:** alongside the reorderable step list, the ordered steps render as a read-only flow diagram (linear chain of step nodes) so Tenant Admins can visually confirm the approval sequence while building or reviewing a template — see `tech_stack.md` §2/§6
...
- The same read-only diagram component used for workflow template preview (§5.5) also renders a running **Workflow Instance**, highlighting the current step and marking completed/rejected steps, so Approvers and Executive Viewers can see progress at a glance
